...
This vignette is intended to provide several examples of different network visualization methods in the `ggplot2` / "tidyverse" framework. Three of the the methods, [the `geomnet` package](https://github.com/sctyner/geomnet), [the `ggnetwork` package](https://github.com/briatte/ggnetwork), and [the function `GGally::ggnet2`](https://github.com/ggobi/ggally), are detailed in the paper *Network Visualization with `ggplot2`* by the authors of this vignette. The fourth method, [the `ggraph` package](https://github.com/thomasp85/ggraph) by Thomas Pedersen has been included for completeness. 

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, cache=TRUE, fig.width=7, fig.height=7.5)
```
...
if (!require(ggraph, quietly = TRUE) ||
    packageVersion("ggraph") < "0.1.1") {
  getFromNamespace("install_github", asNamespace("devtools"))("thomasp85/ggraph")
}
...
## ggraph
```{r madmen_ggraph, echo=TRUE, fig.cap="Mad Men example using the `ggraph` package."}
mm.igraph <- graph_from_data_frame(d = madmen$edges, directed = F, vertices = madmen$vertices)
set.seed(10052016)
ggraph(mm.igraph, layout = 'igraph', algorithm = 'kk') + 
  geom_edge_link(edge_colour = 'grey50') + 
  geom_node_point(aes(colour = Gender), size = 3) + 
  geom_node_text(aes(label = name, color = Gender), size = 3, vjust = -.6) + 
  scale_colour_manual(values = mm.col) +
...
## `ggraph`
```{r blood_ggraph, echo=TRUE, fig.cap="Blood donation example using the `ggraph` package."}
blood.igraph <- graph_from_data_frame(d = blood$edges, directed = T, vertices = unique(blood$vertices[, 1:3]))
set.seed(12252016)
ggraph(blood.igraph, layout = 'igraph', algorithm = 'circle') + 
  geom_edge_link0(edge_colour = 'grey50', arrow = arrow(length = unit(15, "pt"), type = "closed")) + 
  geom_node_point(size = 10, colour = 'darkred') + 
  geom_node_text(aes(label = name), colour = 'grey80') + 
  scale_colour_manual(values = mm.col) +
  ggforce::theme_no_axes() + 
  theme(aspect.ratio = 1)
```
...
## `ggraph`
```{r email_ggraph, echo=TRUE, fig.cap="Blood donation example using the `ggraph` package."}
emailedges <- subset(email$edges, nrecipients <54)
emailedges <- emailedges[, c("From", "to", "day")]
email.igraph <- graph_from_data_frame(d = emailedges, directed = T, vertices = email$nodes)
set.seed(10312016)
ggraph(email.igraph, layout = 'igraph', algorithm = 'fr') + 
  geom_edge_link(edge_alpha = 0.25,
    arrow = arrow(length = unit(5, "pt"),
                  type = "closed")) + 
  geom_node_point(size = 4, aes(color = CurrentEmploymentType)) + 
  geom_node_text(aes(label = name), colour = 'grey80') + 
  scale_colour_manual(values = mm.col) +
  ggforce::theme_no_axes() + 
  theme(aspect.ratio = 1)
